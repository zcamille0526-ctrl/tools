...
5. **二级知识点标题**：用简短、标准化的名词短语，如"开国大典""土地改革""十一届三中全会""香港回归"。不要用描述性长句（错误示例："中华人民共和国成立的历史背景与过程"），要用教材和知识库中常见的标准名称。
6. **二级知识点详情**：简洁、规范、教材化纯文本

{f"## 参考风格{chr(10)}{ref_text}" if ref_text else ""}

## 输出
JSON数组：{{"章":"{chapter}","节":"{section}","一级知识点":"...","二级知识点标题":"...","二级知识点详情":"..."}}
...
def generate_excel(results, output_path):
    ...
    ec = sum(1 for r in results if "已存在" in r.get("备注",""))
    nc = len(results) - ec
    cm = [r for r in results if r.get("_confidence",0) > 0]
    am = [r for r in results if r.get("_confidence",0) == -1]
